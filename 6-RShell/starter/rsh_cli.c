...
int exec_remote_cmd_loop(char *address, int port)
{
    ...
    while (1) {
        ...
        if (send(cli_socket, cmd_buff, cmd_len, 0) != cmd_len) {
            perror("send");
            return client_cleanup(cli_socket, cmd_buff, rsp_buff, ERR_RDSH_COMMUNICATION);
        }

        // If the command is "exit" or "stop-server", break after sending.
        if (strcmp(cmd_buff, "exit") == 0 || strcmp(cmd_buff, "stop-server") == 0) {
            break;
        }

        // Receive the server response. We loop until we detect the EOF marker.
        while (1) {
            ...
            if (io_size == 0) {
                // No bytes received; likely the server closed the connection.
                printf("Server closed connection.\n");
                return client_cleanup(cli_socket, cmd_buff, rsp_buff, ERR_RDSH_COMMUNICATION);
            }
            // Check if the last byte is the EOF marker.
            char *eof_ptr = memchr(rsp_buff, RDSH_EOF_CHAR, io_size);
            if (eof_ptr != NULL) {
                int valid_bytes = eof_ptr - rsp_buff;
                rsp_buff[valid_bytes] = '\0';
                printf("%s", rsp_buff);
                break;
            } else {
                // No EOF marker in this chunk, print all and loop again.
                printf("%.*s", (int)io_size, rsp_buff);
            }
            if (is_eof && (rsp_buff[io_size - 2] != '\n')) {
                printf("\n");
            }
        }
    }
