...
int add_student(int fd, int id, char *fname, char *lname, int gpa){
    ...
    // Make sure the file size is correct by writing a byte at the end if needed
    if (id == 99999) {
        // Calculate the desired file size
        off_t desired_size = 6400000;
        if (lseek(fd, desired_size - 1, SEEK_SET) == -1) {
            return ERR_DB_FILE;
        }
        char null_byte = '\0';
...
int count_db_records(int fd){
    ...
    while (1) {
        ...
        // Check if not empty
        if (memcmp(&student, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) != 0 && student.id != 0) {
            count++;
        }
    }
...
int print_db(int fd){
    ...
    while (1) {
        ...
        // Print header first before record
        if (!header_printed) {
...
int compress_db(int fd){
    ...
    // Seek to beginning of original file
    if (lseek(fd, 0, SEEK_SET) == -1) {
        close(tmp_fd);
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    ...
    while (1) {
        ...
        // Write valid record to temporary file
        off_t offset = (student.id - 1) * STUDENT_RECORD_SIZE;
