...
int string_replace(char *buff, int buff_size, const char *search, const char *replace) {
    ...
    if (new_length >= buff_size) {
        return -1; // Replacement would cause overflow
    }

    if (replace_len != search_len) {
        memmove(pos + replace_len, pos + search_len, remaining_len + 1); // Adjust the buffer
    }

    memcpy(pos, replace, replace_len); // Insert the replacement string
    return 1;
}
